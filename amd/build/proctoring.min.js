...
=======
var isCameraAllowed=!1;define("quizaccess_proctoring/proctoring",["jquery","core/ajax","core/notification"],(function($,Ajax,Notification){function hideButtons(){$(".mod_quiz-next-nav").prop("disabled",!0),$(".submitbtns").html('<p class="text text-red red">You need to enable web camera before submitting this quiz!</p>')}$("#id_submitbutton").prop("disabled",!0),$((function(){$("#id_submitbutton").prop("disabled",!0),$("#id_proctoring").on("change",(function(){this.checked&&isCameraAllowed?$("#id_submitbutton").prop("disabled",!1):$("#id_submitbutton").prop("disabled",!0)}))})),document.querySelector("#screen-share-btn").addEventListener("click",(function(e){e.preventDefault(),console.log("fukc you")}));var takepicturedelay=3e4;return{setup:function(props){if(takepicturedelay=props.camshotdelay,null!==document.getElementById("page-mod-quiz-summary")&&document.getElementById("page-mod-quiz-summary").innerHTML.length)return!1;if(null!==document.getElementById("page-mod-quiz-review")&&document.getElementById("page-mod-quiz-review").innerHTML.length)return!1;var width=props.image_width,height=0,streaming=!1,data=null;$("#mod_quiz_navblock").append('<div class="card-body p-3"><h3 class="no text-left">Webcam</h3> <br/><video id="video">Video stream not available.</video><canvas id="canvas" style="display:none;"></canvas><div class="output" style="display:none;"><img id="photo" alt="The picture will appear in this box."/></div></div>');var video=document.getElementById("video"),canvas=document.getElementById("canvas"),photo=document.getElementById("photo"),takepicture=function(){var context=canvas.getContext("2d");if(width&&height){canvas.width=width,canvas.height=height,context.drawImage(video,0,0,width,height),data=canvas.toDataURL("image/png"),photo.setAttribute("src",data),props.webcampicture=data;var request={methodname:"quizaccess_proctoring_send_camshot",args:{courseid:props.courseid,screenshotid:props.id,quizid:props.quizid,webcampicture:data,imagetype:1}};Ajax.call([request])[0].done((function(res){res.warnings.length<1||video&&Notification.addNotification({message:"Something went wrong during taking the image.",type:"error"})})).fail(Notification.exception)}else!function(){var context=canvas.getContext("2d");context.fillStyle="#AAA",context.fillRect(0,0,canvas.width,canvas.height),data=canvas.toDataURL("image/png"),photo.setAttribute("src",data)}()};return navigator.mediaDevices.getUserMedia({video:!0,audio:!1}).then((function(stream){video.srcObject=stream,video.play(),isCameraAllowed=!0})).catch((function(){hideButtons()})),video?(video.addEventListener("canplay",(function(){streaming||(height=video.videoHeight/(video.videoWidth/width),isNaN(height)&&(height=width/(4/3)),video.setAttribute("width",width),video.setAttribute("height",height),canvas.setAttribute("width",width),canvas.setAttribute("height",height),streaming=!0)}),!1),video.addEventListener("click",(function(ev){takepicture(),ev.preventDefault()}),!1),setTimeout(takepicture,3e3),setInterval(takepicture,takepicturedelay)):hideButtons(),!0},init:function(props){var height=0,streaming=!1,video=null,canvas=null,photo=null,data=null;function clearphoto(){if(isCameraAllowed){var context=canvas.getContext("2d");context.fillStyle="#AAA",context.fillRect(0,0,canvas.width,canvas.height),data=canvas.toDataURL("image/png"),photo.setAttribute("src",data)}else hideButtons()}return video=document.getElementById("video"),canvas=document.getElementById("canvas"),photo=document.getElementById("photo"),document.getElementById("screen-share"),video?(navigator.mediaDevices.getUserMedia({video:!0,audio:!1}).then((function(stream){video.srcObject=stream,video.play(),isCameraAllowed=!0})).catch((function(){Notification.addNotification({message:props.allowcamerawarning,type:"warning"}),hideButtons()})),video.addEventListener("canplay",(function(){streaming||(height=video.videoHeight/(video.videoWidth/width),isNaN(height)&&(height=width/(4/3)),video.setAttribute("width",width),video.setAttribute("height",height),canvas.setAttribute("width",width),canvas.setAttribute("height",height),streaming=!0)}),!1),video.addEventListener("click",(function(ev){!function(){var context=canvas.getContext("2d");if(width&&height){canvas.width=width,canvas.height=height,context.drawImage(video,0,0,width,height),data=canvas.toDataURL("image/png"),photo.setAttribute("src",data);var request={methodname:"quizaccess_proctoring_send_camshot",args:{courseid:props.courseid,screenshotid:props.id,quizid:props.quizid,webcampicture:data,imagetype:1}};Ajax.call([request])[0].done((function(res){res.warnings.length<1||Notification.addNotification({message:"Something went wrong during taking screenshot.",type:"error"})})).fail(Notification.exception)}else clearphoto()}(),ev.preventDefault()}),!1)):hideButtons(),clearphoto(),data}}}));
>>>>>>> 4184084 (cutomize proctoring rule)
